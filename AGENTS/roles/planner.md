...
This role extends the applicable root policy. The root policy owns investigation, authorization, approval, scope, execution, material revision, verification, reporting, and closure. Planner owns the structure of authorization plans, the persistence of saved plans, and the maintenance rules for plans with tracking.
...
- A normal plan is presented, explicitly approved, and executed without tracking artifacts.
...
Persistence is independent of mode. A plan of either mode is saved only when the user explicitly requests it, and a saved plan persists only the artifacts required by its mode. A request to save a plan does not select tracking mode; saving a normal plan persists it as a normal plan.

# Plan Identity

For any saved plan, use a stable, semantic `plan-id` in lowercase kebab-case. It must describe the objective and bounded scope, for example `playwright-coverage-expansion` or `dashboard-configuration-test-remediation`.
...
The directory of a saved plan is `AGENTS/plans/<plan-id>/`. Do not rename a saved plan after approval unless a material revision requires a replacement plan.

# Artifact Structure

A saved normal plan requires exactly these artifacts:
...
Do not create `impact-inventory.md`, `progress-ledger.md`, or `records/` for a saved normal plan.
...
Every `action-plan.md` begins with a `Plan record` containing `Created`, `Saved`, `Approved`, current `Last updated`, and current `Status` fields. `Created` records when the proposed plan was first presented; `Saved` records creation of the persistent plan directory; `Approved` is `Not approved` until the plan is explicitly approved, then records its first approval. Preserve `Created`, `Saved`, and an approval timestamp unchanged once set. For a plan with tracking, it also contains an append-only `Change summary` table with timestamp, concise change, reason, and approval effect. This is the central summary of plan evolution, not a separate version of the plan. A saved normal plan has no `Change summary`; a material revision rewrites its `action-plan.md` in place and does not retain the prior version.
...
A `Draft` has no active implementation unit and must not claim implementation or dependent verification. For a plan with tracking, its ledger identifies explicit approval as its resume point. For a saved normal plan, its `action-plan.md` `Status` and its `resume-prompt.md` approval state identify explicit approval as the requirement to proceed.
...
`resume-prompt.md` is a required derived handoff artifact of every saved plan. It is not an authorization, scope, completion, or verification authority. It must name its repository and give the repo-relative path of every artifact it references, so it resolves its own plan directory without further input. Its first executable instruction must require reading the complete `AGENTS/plans/<plan-id>/action-plan.md`, including `Planning basis` when present, before implementation or dependent verification. It then directs reconciliation with the current working tree and directly observed verification, and for a plan with tracking also with the active ledger entry and the linked owner record when present. Do not duplicate complete stage tables, detailed evidence, execution histories, counts maintained elsewhere, alternative implementation options, or commands that change the working tree or runtime state.

For a plan with tracking, keep it limited to plan identity, repository, artifact paths, derived current state, last closed stage, active or next stage, exact resume point, current blocker, pending user decision, and preserved boundaries. For a saved normal plan, keep it limited to plan identity, repository, artifact paths, approval state, preserved boundaries, and the required reading and reconciliation instructions; do not record stage state or a resume point, which its snapshot persistence cannot keep current.
...
A saved normal plan uses only `Draft`, `Approved`, `Superseded`, and `Complete`. The remaining states require tracking.

Before approval, prepare and present the plan in the user-facing conversation without creating persistent planning artifacts unless the user explicitly requests that the plan be saved. Treat a plan as a plan with tracking only when the user explicitly requests tracking or when the presented plan identifies that mode. When the user explicitly requests that the current unapproved plan be saved, create every artifact required by its mode, including `resume-prompt.md`, with state `Draft`. Saving a plan does not approve, apply, resume, or otherwise authorize it. After explicit approval of a plan with tracking, create every missing required artifact, including `resume-prompt.md`, before dependent implementation begins and change the state to `Approved` unless implementation begins immediately, in which case it is `In progress`. After explicit approval of a saved normal plan, record the approval in `action-plan.md` and `resume-prompt.md` and set its state to `Approved`. Approval of an unsaved normal plan creates no artifacts.
...
For a saved normal plan, update `action-plan.md` and `resume-prompt.md` only when the saved draft changes before approval, the plan is approved, it is materially revised or superseded, or it is closed. Do not record stage progress, evidence, or verification state in a saved normal plan; its current state is established by reconciling it with the current working tree and directly observed verification.

For a plan with tracking, append a `Change summary` entry and update `Last updated` when a draft is saved, tracking starts, a plan is approved, revised, resumed, blocked, partially completed, completed, or when a stage closes. Do not record routine minor actions there. Preserve `Created`, `Saved`, and an approval timestamp unchanged once set. When a material revision supersedes an approved plan, record that the prior approval was replaced and that the revised plan requires explicit approval.
...
When resuming a plan with tracking, inspect the complete action plan, including `Planning basis`, the active ledger entry, linked owner record when present, current working tree, and the evidence needed to establish the next safe action. For a normal plan, reconcile its approved user-visible plan, or its saved `action-plan.md` when the plan was saved, with the current working tree and the evidence needed to establish the next safe action.
...
Before saving any plan as a `Draft`, also verify that its plan-id is semantic, stable, and scoped; that only the artifacts required by its mode exist; that no saved artifact claims unobserved implementation or verification; and that `resume-prompt.md` names its repository and gives the repo-relative path of every artifact it references.

Before saving a plan with tracking as a `Draft`, also verify that its `Planning basis` records the required decision context, is traceable to material evidence, and does not become a parallel research log, and that every tracking artifact has the required operational purpose.
...
- `resume-prompt.md` exists, names its repository, gives the repo-relative path of every artifact it references, and requires complete action-plan reading before execution.
...
- `resume-prompt.md` gives the repo-relative path of every artifact it references, requires complete action-plan reading before execution, and contains no duplicate authority, evidence, scope, or state-changing instruction.
...
For a saved normal plan, mark it `Complete` only after the root completion checks succeed, and record that result in `action-plan.md` and `resume-prompt.md` without claiming unobserved implementation or verification.
