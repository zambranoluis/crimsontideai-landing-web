...
const [beforeDirectory, afterDirectory, destination = "test-results/mesh-performance.md"] = process.argv.slice(2);
...
if (originals.size !== before.length || new Set(after.map(key)).size !== after.length || before.length !== after.length) {
...
const percent = (a, b) => Number.isFinite(a) && Number.isFinite(b) && a > 0 ? `${((b / a - 1) * 100).toFixed(1)}%` : "n/a";
const median = values => {
...
const lines = [
  ...
  "Aggregate costs below are medians of the recorded workload medians, not pooled frame samples. Missing draw measurements remain n/a. Lower drawing time is better; negative changes indicate improvement. Full workloads, frame gaps, long tasks, tiers and bounds are retained in the accompanying JSON and CPU profiles.", "",
  "| Condition | Mesh | Before ms | After ms | Change | Long tasks before → after |",
  "| --- | --- | ---: | ---: | ---: | ---: |",
];
for (const label of ["desktop", "desktop-4x", "mobile-4x"]) for (const variant of ["company", "openjm", "sentinel"]) {
