...
const output = path.join(path.dirname(fileURLToPath(import.meta.url)), 'company-open');
const luminance = rgb => rgb.map(v => v / 255).map(v => v <= .04045 ? v / 12.92 : ((v + .055) / 1.055) ** 2.4).reduce((sum, v, i) => sum + v * [.2126, .7152, .0722][i], 0);
(async () => {
  const browser = await chromium.launch();
  const report = [];
  for (const [width, height] of [[1440, 1000], [1280, 800], [768, 1024], [390, 844], [360, 740]]) {
    ...
    const style = 'header, a[href="#main-content"], nextjs-portal { visibility: hidden !important; }';
  ...
  for (const [width, height] of [[1440, 1000], [1280, 800], [768, 1024], [390, 844], [360, 740]]) {
    ...
    await page.getByTestId('company-mesh').scrollIntoViewIfNeeded();
    await page.getByTestId('company-image').evaluate(image => image.decode());
    await page.locator('section[aria-labelledby="company-heading"]').screenshot({ path: path.join(output, `production-no-js-${width}.png`), style: 'header, a[href="#main-content"], nextjs-portal { visibility: hidden !important; }' });
    await context.close();
  }
