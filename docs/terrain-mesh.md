...
Fine-pointer hover uses the original normalized input, whole-field parallax (`.56`) and localized terrain lift (force `.32`, radius `.18`). Smoothing `.045` at 60Hz becomes `1 - (1 - .045) ** (deltaMs * .06)` so equal elapsed time gives equal movement at 24, 30, 60 and 120Hz. Leaving or cancelling clears hover; the parallax returns smoothly. Offscreen/hidden suspension clears interactions and freezes elapsed animation time.

The existing `MeshInteraction` supplies only the click ripple: 1,200ms, 18 screen-pixel amplitude, 62 screen-pixel band and at most four concurrent waves. Terrain never invokes its attraction hover. Input is measured at most once per rendered interaction frame and clipped to artwork/viewport bounds. Links, native/custom controls, non-primary and keyboard-generated clicks are ignored. Passive listeners on the host and `pointer-events: none` on the artwork preserve native touch scrolling.
