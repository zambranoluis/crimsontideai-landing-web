...
Both mesh types use `MeshScheduler`, `FrameCadence`, `backingSize` and `isMeshControl`, but the footer owns its quality policy so Home/Products defaults remain unchanged. Terrain starts at 92×40 points on fine desktop and 72×32 on narrow/coarse devices. Shared DPR caps are 1.5, 1.25 and 1, with 2160×1440 backing limits. The footer targets 30fps, evaluates 500ms windows, lowers density after two overloaded windows, and falls to 24fps if the low tier remains overloaded. Ten seconds of sustained headroom recover one tier and 30fps.

Hidden or offscreen terrain has no scheduled draw. Reduced motion uses server-rendered phase-zero SVG and does not initialize canvas. Two SVG aspect ratios retain terrain relief on wide and narrow screens, including without JavaScript. Context acquisition/drawing failures expose the same fallback. Context loss permits restoration. Size-only changes retain sampling geometry and dot-bucket buffers; a density-tier change rebuilds them. Hidden resize records pending dimensions for re-entry. Cleanup disconnects lifecycle and resize observers and removes media, resize, context, host and scheduler listeners. The site-wide rule is documented in [animation lifecycle](animation-lifecycle.md).

## Rendering costs

Typed arrays retain points, heights, ribbons, shimmer values, pulse values, static ridges, projection factors and trigonometric terms. Addition identities move invariant trigonometry out of each point calculation. Dots retain the export's exact quantization of radius, color and brightness. Numeric bucket heads/next indices replace per-frame object grids, string-keyed Maps and growing coordinate arrays. Lines use ten depth bands and one transverse pass. Cached viewport bounds cull dots and line segments outside the visible canvas area with 24px halo padding; segments crossing that area remain submitted.
