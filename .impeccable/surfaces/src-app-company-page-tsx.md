...
**Focal motion:** About holds its complete composition stationary on eligible desktops and only the centered particle artwork on responsive layouts while the particles move from brain to gear to bulb with the reader's scroll. Jamaica carries origin through the supplied map. The hero radar introduces regional context; neither map implies deployed infrastructure.
...
**Anti-goals:** no team grid, no founder photography, no timeline of milestones, no press logos, no office imagery presented as scale, no flags or tourism imagery.
...
About introduction, particle track and principles stack through 1024px and on larger devices with a coarse primary pointer. Fine-pointer desktops at least 1025px wide use compact About spacing; there is no fixed 700px eligibility requirement. The complete scene pins only if it fits below the measured header. Jamaica statements use four columns on desktop, two through 1023px, and one below 360px. Below 768px the Jamaica map and closing photograph occupy their own space clear of copy. No tabs, dialogs, hover dependency or hidden principle content.
...
6. **Mobile and tablet:** the introduction scrolls normally, then the intact brain reaches the center of the usable viewport below the measured header. Only the artwork holds there for 2.5 usable viewport heights through brain, gear and bulb, then releases before the three principles in their authored order. Jamaica and closing copy follow. Nothing requires hover, radial layout, tabs, or disclosure.
...
**Decision:** the Company footer lists About CrimsonTide and Built in Jamaica; no Team or Insights labels are displayed.
...
- Hero: `company-radar.png` and its pulse overlay move together in their original 1672 x 941 coordinate system. Both retain responsive alignment and static framing behind the original left-aligned text and contrast overlay. Through 1024px and on coarse-pointer devices, the shared scene proportionally covers the entire hero surface using its container dimensions, centered on the Jamaica region with cropping permitted. Image sizes account for this larger responsive scene. The network image receives the hero preload. Waves run for 6.8 seconds, the sweep for 8.5 seconds; they pause offscreen and in hidden documents. The static map and guide rings remain without JavaScript or with reduced motion. The Discover action remains visible at 1366 x 768.
...
- Responsive artwork-only mode uses a dedicated stationary track whose height is the artwork height plus `2.5 * (viewport height - measured header height)`. Its sticky top is `header height + (usable height - artwork height) / 2`. The artwork retains its 400px mobile / 480px tablet limits and is additionally capped to usable height minus 48px. Progress comes from this track; it stays at brain before the center is reached and at bulb after release. The introduction and principles never pin.
- Mode selection is full-scene fit first, artwork-only second, normal flow last. When the full scene cannot fit, artwork-only remains eligible with at least 288px usable height: 48px reserved around at least 240px of artwork. Below that threshold, or with reduced motion, no JavaScript or unavailable/lost canvas, About stays in normal flow without added scroll distance. Animated normal-flow progress retains `(0.85 * viewport height - artwork top) / (artwork height + 0.60 * viewport height)`, clamped to 0–1. Geometry resynchronizes for scene/header resizing, fonts, restored scroll, fragments and motion-preference changes. All local subscriptions are disposed on unmount.
...
- Jamaica: `company-tecnology.png` with its island focal point to the right on desktop. The four displayed statements are “Founded in Jamaica”, “Developing technology from the Caribbean”, “Proprietary products, software & solutions”, and “For organisations worldwide”. They use the supplied target, Jamaica, proprietary-technology and world icons; they have no interaction semantics.
...
This dated local evidence records checks run during the Company implementation. Referenced capture paths are historical local locations, absent from this checkout, not working repository links. It is not current browser coverage or proof for later changes; the behavior above and the current implementation remain authoritative.
...
The explicit Impeccable detector reported palette advisories for supplied animation colors and type advisories for the existing responsive sizes, including the approved compact 22px principle headings. Generated design metadata was outside that historical task; this documentation reconciliation refreshes it separately.
