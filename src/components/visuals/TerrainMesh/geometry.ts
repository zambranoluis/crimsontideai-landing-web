...
export class TerrainGeometry {
  ...
  constructor(readonly columns: number, readonly rows: number) {
    ...
    // x01, z, projected x/y factors, lift, ridges, then six trig pairs.
    this.terms = new Float64Array(count * 18);
    for (let r = 0; r < rows; r++) for (let c = 0; c < columns; c++) {
      const i = (r * columns + c) * 18, x01 = c / (columns - 1), z = r / (rows - 1), nx = x01 * 2 - 1;
      ...
      this.terms[i + 5] = Math.exp(-((x01 - p.ridge1X) ** 2 / (.032 * p.ridge1Width ** 2) + (z - p.ridge1Y) ** 2 / (.07 * p.ridge1Depth ** 2))) * p.ridge1Height
      ...
      for (let j = 0; j < 6; j++) { this.terms[i + 6 + j * 2] = Math.sin(angles[j]); this.terms[i + 7 + j * 2] = Math.cos(angles[j]); }
    }
  }
  ...
  project(width: number, height: number, elapsed: number) {
    ...
    const a = this.terms, s = this.phases;
    for (let r = 0; r < this.rows; r++) {
      ...
      for (let c = 0; c < this.columns; c++) {
        const index = r * this.columns + c, i = index * 18, x = a[i];
        const broad = (a[i + 6] * s[1] + a[i + 7] * s[0]) * .44 + (a[i + 9] * s[3] - a[i + 8] * s[2]) * .28;
        const detail = ((a[i + 10] * s[5] + a[i + 11] * s[4]) * .16 + (a[i + 13] * s[7] - a[i + 12] * s[6]) * .1 + (a[i + 14] * s[9] + a[i + 15] * s[8]) * .045) * p.detail;
        const terrain = (broad + detail + a[i + 5]) * p.waveHeight;
        this.heights[index] = terrain;
        this.points[index * 2] = width * a[i + 2];
        this.points[index * 2 + 1] = height * a[i + 3] - terrain * a[i + 4];
        this.ribbons[index] = Math.max(Math.exp(-((x - b1) ** 2) / .01) * .72, Math.exp(-((x - b2) ** 2) / .011), Math.exp(-((x - b3) ** 2) / .009) * .9);
        this.shimmers[index] = .5 + .5 * (a[i + 16] * s[11] + a[i + 17] * s[10]);
      }
    }
    return this.points;
  }
